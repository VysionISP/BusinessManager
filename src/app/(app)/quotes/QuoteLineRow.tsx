...
import { quoteLineMarkupPercent, quoteLineSell, type QuoteLineLike } from "@/lib/calculations";
...
export function QuoteLineRow({
  ...
  const lineLike: QuoteLineLike = line;
      ...
      <Tr>
        <td colSpan={9} className="p-0">
          <form
            ...
            className="grid grid-cols-2 gap-2 border-y border-indigo-100 bg-indigo-50/40 p-3 sm:grid-cols-7 sm:items-end dark:border-indigo-500/20 dark:bg-indigo-500/5"
          >
            ...
            <FormField label="Cost ($)" name="unitCost" type="number" step="0.01" defaultValue={line.unitCost} />
            <FormField label="Price ($)" name="unitPrice" type="number" step="0.01" defaultValue={line.unitPrice} />
            <FormField label="Tax (%)" name="taxPercent" type="number" step="0.1" defaultValue={line.taxPercent} />
            <FormField label="Discount (%)" name="discountPercent" type="number" step="0.1" defaultValue={line.discountPercent} />
  ...
  return (
    <Tr>
      ...
      <Td className="text-slate-400">{formatPercent(quoteLineMarkupPercent(lineLike), 1)}</Td>
      <Td className="text-slate-500 dark:text-slate-400">{line.taxPercent > 0 ? `${line.taxPercent}%` : "—"}</Td>
      <Td className="font-medium">{formatCurrency(quoteLineSell(lineLike))}</Td>
